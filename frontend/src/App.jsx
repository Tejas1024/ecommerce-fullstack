...
import { ShoppingCart, User, LogOut, Package, TrendingUp, DollarSign, AlertCircle, Search, Filter, Download, Edit, Trash2, Plus, Eye } from 'lucide-react';
...
// User App Wrapper
function UserApp({ user, token, cart, onLogout, currentPage, setCurrentPage, onAddToCart, onLoadCart }) {
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto px-4 py-8">
        {currentPage === 'home' && <ProductList token={token} onAddToCart={onAddToCart} />}
        {currentPage === 'cart' && <CartPage cart={cart} token={token} onLoadCart={onLoadCart} setCurrentPage={setCurrentPage} />}
            ...
            <div key={product.id} className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition">
              ...
              <div className="p-4">
                ...
                <button
                  ...
                  className="w-full bg-indigo-600 text-white py-2 rounded-lg hover:bg-indigo-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition"
                >
...
function AdminPanel({ user, token, onLogout, currentPage, setCurrentPage }) {
  return (
    <div className="min-h-screen bg-gray-50">
      <AdminHeader user={user} onLogout={onLogout} currentPage={currentPage} setCurrentPage={setCurrentPage} />
      <div className="flex">
...
// Admin Header
function AdminHeader({ user, onLogout, currentPage, setCurrentPage }) {
  return (
...
// Product Form
function ProductForm({ token, categories, product, onClose }) {
  const [formData, setFormData] = useState({
    name: product?.name || '',
    slug: product?.slug || '',
...
function AdminOrders({ token }) {
  ...
  if (viewOrder) {
    return <OrderDetail order={viewOrder} token={token} onBack={() => setViewOrder(null)} onStatusUpdate={handleStatusUpdate} />;
  }
...
// Order Detail
function OrderDetail({ order, token, onBack, onStatusUpdate }) {
